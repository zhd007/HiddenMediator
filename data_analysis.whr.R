...
#### ---------------------------- Step1: regressions ---------------------------- ####
data <- read.csv("/Users/dr/Desktop/ms_thesis/traits/cleaned_pheno_geno_data.whr.csv", stringsAsFactors = FALSE)
dim(data)   # 216411    538
colnames(data)

first_snp_index <- 17
colnames(data)[first_snp_index:length(colnames(data))]
(num_snps <- length(colnames(data)) - first_snp_index + 1)   # 522

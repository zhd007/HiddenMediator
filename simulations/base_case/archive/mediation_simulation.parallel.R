...
mediation_simulation <- function(true_hidden) {
  ...
  tryCatch({
    ...
    betaS1 <- rnorm(n = num_snp, mean = upper_level_effect[1], sd = 0.05)
    #betaS1 <- betaS1_2[,1]
    ...
    betaS2 <- rnorm(n = num_snp, mean = upper_level_effect[2], sd = 0.05)
    #betaS2 <- betaS1_2[,2]
    ...
    betaS3 <- rnorm(n = num_snp, mean = upper_level_effect[3], sd = 0.05)
    ...
    betaS4 <- rnorm(n = num_snp, mean = upper_level_effect[4], sd = 0.05)
    ...
    betaS5 <- rnorm(n = num_snp, mean = upper_level_effect[5], sd = 0.05)
    ...
    betaS6 <- rnorm(n = num_snp, mean = upper_level_effect[6], sd = 0.05)
    ...
    # 
    # mu[2] = bH * mu2
    # mu2 ~ dnorm(mu_a, 1/(sd_a2^2)) T(min_a,max_a)
    # mu[1] ~ dnorm(0, 1/(sd_c1^2)) T(min_c,mu[2])
    # 
    # prec[1] ~ dgamma(1/1000, 1/1000)
    # prec[2] ~ dgamma(1/1000, 1/1000)
    # 
    # 
    # bH ~ dnorm(0, 1/100)
    # }"
    ...
    data_jags <- list(c = all_c, mu_a = mu_a, 
                      sd_a2 = sd_a2, sd_c1 = sd_c1,
                      n = length(all_c),
                      pi = pi,
                      max_a = max(all_a), min_a = min(all_a),
                      min_c = min(all_c)  )
    
    inits_jags <- list(bH = 0, prec = c(0.001,0.001))
    ...
    # 95% quantile CI
    b.low1 <- quantile(mod_csim[,1], 0.025)
    b.high1 <- quantile(mod_csim[,1], 0.975)
    
    # 89% HDI
    b.low2 <- hdi(mod_csim[,1], credMass = 0.89)[1]
    b.high2 <- hdi(mod_csim[,1], credMass = 0.89)[2]
  ...
  return(   list("temp.record" = temp.record, "a_coefs" = a_coefs, "c_coefs" = a_coefs) )
}


#ptm <- proc.time()    # start timer
sim_result <- mclapply(X = true_hidden_vector, FUN = mediation_simulation, mc.cores = 6)



## gather simulation results
a_coefs <- vector(mode = "list", length = set)
c_coefs <- vector(mode = "list", length = set)
record <- data.frame("freq" = rep(-9, set),  "true_freq" = rep(-9, set), 
                     "pi_hat" = rep(-9, set), 
                     "real_a_mean" = rep(-9, set), "real_c_mean" = rep(-9, set),
                     "b_hat.median" = rep(-9, set), "b_hat.mean" = rep(-9, set),
                     "b.low1" = rep(-9, set), "b.high1" = rep(-9, set),
                     "b.low2" = rep(-9, set), "b.high2" = rep(-9, set),
                     "b.low3" = rep(-9, set), "b.high3" = rep(-9, set),
                     "true_hidden" = rep(-9, set))
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous/base_case/a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous/base_case/c_coefs.csv")
# write.csv(data.frame(matrix(unlist(bH_path), nrow=length(bH_path), byrow=T)), row.names = FALSE,
#           "/Users/dr/Desktop/mediation_summary/simulation_sets/snp70.new_priors.pi0.5.single_mode/bH_path.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous/base_case/record.csv")

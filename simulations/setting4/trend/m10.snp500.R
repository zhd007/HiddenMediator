...
mediation_simulation <- function(true_hidden) {
  ...
  em.times <- 15
  known_freq <- c(0.5, 0.6, 0.8, 0.2, 0.5, 0.8 ,0.3, 0.4, 0.5, 0.6, 0.8)    # freq of m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m_Hidden
  known_beta <- c(0.4, 0.2, 0.3, 0.2, 0.4, 0.2, 0.1, 0.3, 0.2, 0.2)            # beta1 and beta2, beta3, beat4, beta5
  sample_size <- 100000
  ...
  tryCatch({
    ...
    m6 <- m6[,1]
    m6 <- m6 + 15 # constant
    m6 <- m6 + rnorm(sample_size, mean = 0, sd = 1.5)
    #m6 <- m6 + runif(sample_size, min = -0.4, max = 0.4)

    ############# known: m7
    seq7 <- sample(c(0, 1), size = num_snp, replace = TRUE, prob = c(1-known_freq[7], known_freq[7]))
    #seq7 <- rep(1, num_snp)
    # a vector of beta(snp->mediator)
    #betaS7 <- rnorm(n = num_snp, mean = 0.3, sd = 0.1)
    betaS7 <- rnorm(n = num_snp, mean = upper_level_effect[7], sd = 0.08)

    m7 <- data.matrix(df.snp) %*% (seq7 * betaS7)
    m7 <- m7[,1]
    m7 <- m7 + 10 # constant
    m7 <- m7 + rnorm(sample_size, mean = 0, sd = 1)
    #m7 <- m7 + runif(sample_size, min = -0.4, max = 0.4)

    ############# known: m8
    seq8 <- sample(c(0, 1), size = num_snp, replace = TRUE, prob = c(1-known_freq[8], known_freq[8]))
    #seq8 <- rep(1, num_snp)
    # a vector of beta(snp->mediator)
    #betaS8 <- rnorm(n = num_snp, mean = 0.3, sd = 0.1)
    betaS8 <- rnorm(n = num_snp, mean = upper_level_effect[8], sd = 0.08)

    m8 <- data.matrix(df.snp) %*% (seq8 * betaS8)
    m8 <- m8[,1]
    m8 <- m8 + 5 # constant
    m8 <- m8 + rnorm(sample_size, mean = 0, sd = 0.5)
    #m8 <- m8 + runif(sample_size, min = -0.4, max = 0.4)
    
    ############# known: m9
    seq9 <- sample(c(0, 1), size = num_snp, replace = TRUE, prob = c(1-known_freq[9], known_freq[9]))
    #seq9 <- rep(1, num_snp)
    # a vector of beta(snp->mediator)
    #betaS9 <- rnorm(n = num_snp, mean = 0.3, sd = 0.1)
    betaS9 <- rnorm(n = num_snp, mean = upper_level_effect[9], sd = 0.08)

    m9 <- data.matrix(df.snp) %*% (seq9 * betaS9)
    m9 <- m9[,1]
    m9 <- m9 + 2 # constant
    m9 <- m9 + rnorm(sample_size, mean = 0, sd = 0.5)
    #m9 <- m9 + runif(sample_size, min = -0.4, max = 0.4)

    ############# known: m10
    seq10 <- sample(c(0, 1), size = num_snp, replace = TRUE, prob = c(1-known_freq[10], known_freq[10]))
    #seq10 <- rep(1, num_snp)
    # a vector of beta(snp->mediator)
    #betaS10 <- rnorm(n = num_snp, mean = 0.3, sd = 0.1)
    betaS10 <- rnorm(n = num_snp, mean = upper_level_effect[10], sd = 0.09)

    m10 <- data.matrix(df.snp) %*% (seq10 * betaS10)
    m10 <- m10[,1]
    m10 <- m10 + 20 # constant
    m10 <- m10 + rnorm(sample_size, mean = 0, sd = 1)
    #m10 <- m10 + runif(sample_size, min = -0.4, max = 0.4)

    ############# hidden: m11
    seq11 <- sample(c(0, 1), size = num_snp, replace = TRUE, prob = c(1-known_freq[11], known_freq[11]))
    # a vector of beta(snp->mediator)
    #betaS11 <- rnorm(n = num_snp, mean = 0.3, sd = 0.1)
    betaS11 <- rnorm(n = num_snp, mean = upper_level_effect[11], sd = 0.08)

    m11 <- data.matrix(df.snp) %*% (seq6 * betaS6)
    m11 <- m11[,1]
    m11 <- m11 + 20 # constant
    m11 <- m11 + rnorm(sample_size, mean = 0, sd = 1)
    ...
    y <- known_beta[1] * m1 + known_beta[2] * m2 + known_beta[3] * m3 +
      known_beta[4] * m4 + known_beta[5] * m5 + known_beta[6] * m6 +
      known_beta[7] * m7 + known_beta[8] * m8 + known_beta[9] * m9 +
      known_beta[10] * m10 +
      true_hidden * m11 +
    ...
    beta_m6 <- vector(mode = "logical", length = num_snp)
    beta_m7 <- vector(mode = "logical", length = num_snp)
    beta_m8 <- vector(mode = "logical", length = num_snp)
    beta_m9 <- vector(mode = "logical", length = num_snp)
    beta_m10 <- vector(mode = "logical", length = num_snp)
    ...
    eq.m7.all <- paste("m7~", snp_sum.cat, sep = "")
    eq.m8.all <- paste("m8~", snp_sum.cat, sep = "")
    eq.m9.all <- paste("m9~", snp_sum.cat, sep = "")
    eq.m10.all <- paste("m10~", snp_sum.cat, sep = "")
    eq.m11.all <- paste("m11~", snp_sum.cat, sep = "")    
    ...
    m7_model <- lm(eq.m7.all, data = df.snp)
    m8_model <- lm(eq.m8.all, data = df.snp)
    m9_model <- lm(eq.m9.all, data = df.snp)
    m10_model <- lm(eq.m10.all, data = df.snp)
    m11_model <- lm(eq.m11.all, data = df.snp)
    ...
    m7_coefs <- summary(m7_model)$coefficient
    m8_coefs <- summary(m8_model)$coefficient
    m9_coefs <- summary(m9_model)$coefficient
    m10_coefs <- summary(m10_model)$coefficient
    m11_coefs <- summary(m11_model)$coefficient
    ...
    df.a <- data.frame("a1.alt" = m1_coefs[2:(num_snp+1),1],
                       ...
                       "a7.alt" = m7_coefs[2:(num_snp+1),1],
                       "a8.alt" = m8_coefs[2:(num_snp+1),1],
                       "a9.alt" = m9_coefs[2:(num_snp+1),1],
                       "a10.alt" = m10_coefs[2:(num_snp+1),1],
                       "a11.alt" = m11_coefs[2:(num_snp+1),1],
                       ...
                       "a7.var.alt" = m7_coefs[2:(num_snp+1),2]^2,
                       "a8.var.alt" = m8_coefs[2:(num_snp+1),2]^2,
                       "a9.var.alt" = m9_coefs[2:(num_snp+1),2]^2,
                       "a10.var.alt" = m10_coefs[2:(num_snp+1),2]^2,
                       "a11.var.alt" = m11_coefs[2:(num_snp+1),2]^2)
    ...
    eq.y.all <- paste("y~m1+m2+m3+m4+m5+m6+m7+m8+m9+m10+", snp_sum.cat, "+covariate1+covariate2", sep = "")
    ...
    df.b <- data.frame("b1.alt" = y_coefs[2,1], "b2.alt" = y_coefs[3,1], "b3.alt" = y_coefs[4,1], 
                       "b4.alt" = y_coefs[5,1], "b5.alt" = y_coefs[6,1], "b6.alt" = y_coefs[7,1], 
                       "b7.alt" = y_coefs[8,1], "b8.alt" = y_coefs[9,1], "b9.alt" = y_coefs[10,1],
                       "b10.alt" = y_coefs[11,1],
    ...
    all_a <- c(df.a$a1.alt,df.a$a2.alt, df.a$a3.alt, df.a$a4.alt, df.a$a5.alt,
               df.a$a6.alt,df.a$a7.alt, df.a$a8.alt, df.a$a9.alt, df.a$a10.alt)
    all_c <- df.b$c.alt
    ...
    inits_jags <- list(bH = 0, .RNG.name = "base::Wichmann-Hill", ".RNG.seed" = 400)
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting4/trend/m10.snp500.a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting4/trend/m10.snp500.c_coefs.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting4/trend/m10.snp500.record.csv")

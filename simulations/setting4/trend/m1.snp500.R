...
mediation_simulation <- function(true_hidden) {
  ...
  em.times <- 15
  known_freq <- c(0.5, 0.8)    # freq of m1,  m_Hidden
  known_beta <- c(0.4)            # beta1 
  sample_size <- 100000
  ...
  tryCatch({
    ...
    #m2 <- m2 + runif(sample_size, min = -0.3, max = 0.3)
    ...
    ############## mediator -> outcome
    y <- known_beta[1] * m1 + 
      true_hidden * m2 + rnorm(sample_size, mean = 0, sd = 0.2) +
    ...
    real_c_mean <- mean(betaS1[which(seq2 == 1)] * true_hidden)
    ...
    eq.y.all <- paste("y~m1+", snp_sum.cat, "+covariate1+covariate2", sep = "")
    
    outcome_model <- lm(eq.y.all, data = df.snp)
    y_coefs <- summary(outcome_model)$coefficient
    
    
    df.b <- data.frame("b1.alt" = y_coefs[2,1],
    ...
    inits_jags <- list(bH = 0, .RNG.name = "base::Wichmann-Hill", ".RNG.seed" = 410)
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting4/trend/m1.snp500.a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting4/trend/m1.snp500.c_coefs.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting4/trend/m1.snp500.record.csv")

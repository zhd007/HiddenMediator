...
mediation_simulation <- function(true_hidden) {
  ...
  tryCatch({
    ...
    upper_level_effect <- rnorm(n = length(known_beta)+1, 0.2, 0.02)
    ...
    betaS1 <- rnorm(n = num_snp, mean = upper_level_effect[1], sd = 0.06)
    #betaS1 <- betaS1_2[,1]
    ...
    betaS2 <- rnorm(n = num_snp, mean = upper_level_effect[2], sd = 0.06)
    #betaS2 <- betaS1_2[,2]
    ...
    betaS3 <- rnorm(n = num_snp, mean = upper_level_effect[3], sd = 0.06)
    ...
    betaS4 <- rnorm(n = num_snp, mean = upper_level_effect[4], sd = 0.06)
    ...
    betaS5 <- rnorm(n = num_snp, mean = upper_level_effect[5], sd = 0.06)
    ...
    betaS6 <- rnorm(n = num_snp, mean = upper_level_effect[6], sd = 0.06)
    ...
    inits_jags <- list(bH = 0, .RNG.name = "base::Wichmann-Hill", ".RNG.seed" = 600)
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting6/trend/sd1-3.snp500.a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting6/trend/sd1-3.snp500.c_coefs.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting6/trend/sd1-3.snp500.record.csv")

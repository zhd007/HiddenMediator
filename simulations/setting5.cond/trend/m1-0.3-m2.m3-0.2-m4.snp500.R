...
mediation_simulation <- function(true_hidden) {
  ...
  tryCatch({
    ...
    m2 <-  m2 + 0.3*m1                                       # add some of m1 to m2
    #m2 <- m2 + cor12[,2]
    ...
    eq.m2.all <- paste("m2~", snp_sum.cat, "+m1", sep = "")
    eq.m3.all <- paste("m3~", snp_sum.cat, sep = "")
    eq.m4.all <- paste("m4~", snp_sum.cat, "+m3", sep = "")
    ...
    inits_jags <- list(bH = 0, .RNG.name = "base::Wichmann-Hill", ".RNG.seed" = 520)
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting5.cond/trend/m1-0.3-m2.m3-0.2-m4.snp500.a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting5.cond/trend/m1-0.3-m2.m3-0.2-m4.snp500.c_coefs.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting5.cond/trend/m1-0.3-m2.m3-0.2-m4.snp500.record.csv")

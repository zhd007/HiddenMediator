...
mediation_simulation <- function(true_hidden) {
  ...
  em.times <- 15
  known_freq <- c(0.5, 0.6, 0.8, 0.2, 0.5, 0.5 )    # freq of m1, m2, m3, m4, m5, m_Hidden
  ...
  tryCatch({
    ...
    inits_jags <- list(bH = 0, .RNG.name = "base::Wichmann-Hill", ".RNG.seed" = 110)
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting1/trend/piH0.5.snp500.a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting1/trend/piH0.5.snp500.c_coefs.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting1/trend/piH0.5.snp500.record.csv")

...
mediation_simulation <- function(true_hidden) {
  ...
  tryCatch({
    ...
    inits_jags <- list(bH = 0, .RNG.name = "base::Wichmann-Hill", ".RNG.seed" = 200)
...
write.csv(data.frame(matrix(unlist(a_coefs), nrow=length(a_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting2/trend/neg2.snp500.a_coefs.csv")
write.csv(data.frame(matrix(unlist(c_coefs), nrow=length(c_coefs), byrow=T)), row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting2/trend/neg2.snp500.c_coefs.csv")

# save record dataframe
write.csv(record, row.names = FALSE,
          "/home/dingzh/t2d_mediation/continuous.updated/setting2/trend/neg2.snp500.record.csv")
